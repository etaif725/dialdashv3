...
export class LeadsService {
  ...
  async updateLeadStatus(ids: string[], status: Lead["call_status"]): Promise<{ success: boolean; data?: Lead[] | null; error?: any }> {
    try {
      const { data, error } = await this.supabase
        .from('leads')
  ...
  async updateCallStatus(phoneNumber: string, status: Lead[]): Promise<{ success: boolean; error?: any }> {
    try {
      const { error } = await this.supabase
        .from('leads')
        .update({ 
